...
public sealed class PipelineExecutor
{
    ...
    private object ParseArgs(CommandDescriptor desc, string segment)
    {
        ...
        {
            // 无参数：用默认值构造。
            var ctor = desc.ArgsType.GetConstructors().First();
            var parameters = ctor.GetParameters();
            var argsValues = new object?[parameters.Length];
            for (int i = 0; i < parameters.Length; i++)
                argsValues[i] = parameters[i].DefaultValue;
            return ctor.Invoke(argsValues) ?? throw new InvalidOperationException("Args constructor returned null.");
        }
        // 复用 CliHost 的 ParseArgs 反射逻辑（简化版，M2 后续可抽到共享工具）。
        return ParseArgsReflection(desc, tokens.Skip(1).ToArray());
    }

    private object ParseArgsReflection(CommandDescriptor desc, string[] tokens)
    {
        ...
        foreach (var p in desc.Parameters)
        {
            if (p.Type == typeof(bool))
            {
                ...
                foreach (var a in p.Aliases ?? Array.Empty<string>())
